...
export default function MessagePage() {
  ...
  useEffect(() => {
    ...
    const loadConversation = async () => {
      ...
      try {
        const conversation = await driverMessageService.getConversationById(conversationId);
        if (conversation) {
          setCurrentConversation(conversation);
          driverMessageSocketService.joinConversation(conversationId);

          const loadedMessages = await driverMessageService.getMessages(conversationId);
          setMessages(conversationId, loadedMessages);

          await driverMessageService.markAsRead(conversationId);
    ...
    return () => {
      if (conversationId) {
        driverMessageSocketService.leaveConversation(conversationId);
      }
    };
  ...
  useEffect(() => {
    ...
    const unsubscribe = driverMessageSocketService.onNewMessage((message, conversation) => {
      if (conversation.id === conversationId) {
        ...
        if (message.sender_id !== user?.id) {
          driverMessageService.markAsRead(conversationId);
          markAsRead(conversationId);
        }
      }
    });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const realMessage = await driverMessageService.sendMessage(conversationId, content);
