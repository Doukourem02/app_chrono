    ...
    try {
      ...
      this.socket.on('order:status:update', (data: OrderStatusUpdateData) => {
        ...
        if (data.order?.status === 'pending' && typeof window !== 'undefined') {
          ...
          if (process.env.NODE_ENV === 'development') {
            logger.debug('[adminSocketService] 📦 Commande avec status pending détectée:', {
          ...
          if (isClientOrder) {
            if (process.env.NODE_ENV === 'development') {
              logger.debug('[adminSocketService] Commande client détectée, tentative de jouer le son pour nouvelle commande:', data.order?.id)
            }
            import('@/utils/soundService').then(({ soundService }) => {
              if (process.env.NODE_ENV === 'development') {
                logger.debug('[adminSocketService] soundService importé (order:status:update), appel de playNewOrder()')
              }
              soundService.playNewOrder().then(() => {
                if (process.env.NODE_ENV === 'development') {
                  logger.debug('[adminSocketService] playNewOrder() résolu avec succès (order:status:update)')
                }
              }).catch((err) => {
                logger.warn('[adminSocketService] Erreur lecture son nouvelle commande (order:status:update):', err)
              })
            }).catch((err) => {
              logger.warn('[adminSocketService] Erreur chargement soundService (order:status:update):', err)
            })
          } else {
            if (process.env.NODE_ENV === 'development') {
              logger.debug('[adminSocketService] Commande téléphonique, son ignoré (order:status:update)')
            }
          }
        }
      ...
      this.socket.on('order:created', (data: OrderCreatedData) => {
        if (process.env.NODE_ENV === 'development') {
          logger.debug('[adminSocketService] 📨 ÉVÉNEMENT order:created REÇU!', {
            hasData: !!data,
            hasOrder: !!data?.order,
            orderId: data?.order?.id,
            timestamp: new Date().toISOString(),
        ...
        if (isClientOrder && typeof window !== 'undefined') {
          if (process.env.NODE_ENV === 'development') {
            logger.debug('[adminSocketService] Commande client détectée, tentative de jouer le son pour order:created:', data.order?.id)
          }
          // Importer et jouer le son immédiatement (sans délai)
          import('@/utils/soundService').then(({ soundService }) => {
            if (process.env.NODE_ENV === 'development') {
              logger.debug('[adminSocketService] soundService importé, appel de playNewOrder()')
            }
            // Jouer le son immédiatement, sans attendre
            soundService.playNewOrder().catch((err) => {
              logger.warn('[adminSocketService] Erreur lecture son order:created:', err)
            })
          }).catch((err) => {
            logger.warn('[adminSocketService] Erreur chargement soundService:', err)
          })
        } else {
          if (process.env.NODE_ENV === 'development') {
            logger.debug('[adminSocketService] Son ignoré:', {
              isClientOrder,
              hasWindow: typeof window !== 'undefined',
              isPhoneOrder: data.order?.is_phone_order,
            })
