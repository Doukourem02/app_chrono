...
function isDeletedProjectError(error: Error | string | unknown): boolean {
  ...
  return deletedProjectKeywords.some(
    (keyword) =>
      errorMessage.includes(keyword) ||
      errorName.includes(keyword) ||
      errorString.includes(keyword)
...
export function GoogleMapsProvider({ children }: { children: ReactNode }) {
  ...
  const [runtimeError, setRuntimeError] = useState<Error | undefined>(
    undefined
  );
  ...
  const googleMapsApiKey = useMemo(
    () => directApiKey || googleConfig?.apiKey || "",
    [directApiKey, googleConfig?.apiKey]
  );
  ...
  useEffect(() => {
    if (loadError) {
      if (isBillingError(loadError) || isDeletedProjectError(loadError)) {
  ...
  useEffect(() => {
    ...
    const handleError = (event: ErrorEvent) => {
      const errorMessage =
        event.message || event.error?.message || String(event.error || "");
      if (
        errorMessage &&
        (isBillingError(errorMessage) || isDeletedProjectError(errorMessage))
      ) {
      ...
      const errorString = args
        .map((arg) => {
          if (arg instanceof Error)
            return arg.message || arg.name || String(arg);
