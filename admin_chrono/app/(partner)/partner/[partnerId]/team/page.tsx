...
function InviteModal({ partnerId, onClose, onInvited }: { partnerId: string; onClose: () => void; onInvited: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    setError('')
    const result = await partnerApiService.inviteTeamMember(partnerId, { email: email.trim(), role: 'manager' })
    setLoading(false)
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.4)' }}>
      <div style={{ backgroundColor: themeColors.cardBg, borderRadius: 16, padding: 28, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, color: themeColors.textPrimary, marginBottom: 20 }}>Inviter un manager</h2>
        {success ? (
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ padding: '10px 14px', borderRadius: 8, backgroundColor: themeColors.blueLight, border: `1px solid ${themeColors.bluePrimary}` }}>
              <p style={{ fontSize: 12, color: themeColors.bluePrimary }}>
                {"Le manager pourra créer des commandes et suivre les livraisons. Seul le propriétaire peut gérer la facturation et l'équipe."}
              </p>
            </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        >
          <UserPlus size={16} /> Inviter un manager
        </button>
      </div>
                  ...
                  <tr key={m.id} style={{ borderBottom: `1px solid ${themeColors.cardBorder}` }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{
                        ...
                        backgroundColor: m.role === 'owner' ? themeColors.purpleLight : themeColors.blueLight,
                        color: m.role === 'owner' ? themeColors.purplePrimary : themeColors.bluePrimary,
                      }}>
