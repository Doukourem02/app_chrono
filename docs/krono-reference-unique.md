...
**6. Le Dynamic Island / Live Activity (section 4) n'avait aucun signal de dégradation — audit dédié demandé par l'utilisateur, plus grave que le point 1 (bloquant, corrigé le 2026-07-29).**
- Contexte : l'utilisateur a demandé un avis sur la propreté de l'implémentation Dynamic Island. Verdict global : bon travail, contrat produit (section 4) respecté voire dépassé (fallback push classique toujours actif, tokens APNs invalides bien nettoyés, pas de Live Activity fantôme) — un seul vrai trou trouvé, structurel.
- Constat : `notifyLiveActivitiesForDriverLocation` (`liveActivityApnsService.ts`) n'est appelée que depuis le handler `order:driver:location`, déclenché uniquement par une position socket reçue du livreur — aucun heartbeat, aucun autre appelant. Si le livreur perd sa connexion, le Dynamic Island/écran verrouillé continue d'afficher indéfiniment le dernier ETA et la dernière position connus, sans aucune indication — plus trompeur que le point 1 car c'est une surface à très haute autorité visuelle (écran verrouillé, notch), montrée avec assurance.
- Fix : nouveau champ `connectionDegraded` sur les props Live Activity (backend `liveActivityApnsService.ts` + widget `orderTrackingLiveActivity.tsx`, affiché "Signal instable" en bannière verrouillage et vue étendue). `notifyLiveActivityConnectionState` (nouvelle fonction) poussée via APNs depuis `notifyClientsOfDriverConnectionState` (orderSocket.ts, même point que le fix client du point 1) — donc déclenchée même app livreur en arrière-plan, son cas d'usage principal. Répercuté aussi côté client premier plan (`orderLiveActivity.ts`, `useOrderLiveActivitySync.ts`) via `useOrderStore.driverConnection`, avec le court-circuit de dédoublonnage (`shouldSkipLiveActivityUpdate`) corrigé pour ne pas avaler ce changement d'état.
- Point mineur non corrigé (risque de maintenance, pas de bug actif) : `progressFromDriverMovement` est dupliquée quasi à l'identique entre client (`orderLiveActivity.ts`) et backend (`liveActivityApnsService.ts`) — pas de divergence trouvée aujourd'hui, mais un correctif futur appliqué d'un seul côté romprait silencieusement la cohérence premier plan / push APNs arrière-plan.

Statut : les 6 points sont corrigés et vérifiés (`tsc` propre sur les 4 apps, tests backend verts). Pas de décision ouverte supplémentaire issue de cet audit — les 2 déjà notées en section 14 restent les seules côté multi-ville/carte.
