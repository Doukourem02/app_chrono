...
- `**admin_krono/migrations/`** : flotte (`fleet_vehicles`, carburant, entretien, etc.) + scripts RLS. À exécuter **en plus** selon la doc ou l'ordre métier (souvent après les tables `users` / `driver_profiles`).
...
Pour une **nouvelle base vide** : enchaîne **001 → 020 → 021 → 016_add_driver_client_info_to_transactions.sql → 022 → 023 → 024 → 025 → … → 041** (respecte la note ci-dessus sur l'ordre réel de `016_add_driver_client_info_to_transactions.sql`), puis **admin_krono** + **RLS** si besoin. Ce chemin n'a pas encore été validé de bout en bout sur une base réellement vide (voir section suivante).
