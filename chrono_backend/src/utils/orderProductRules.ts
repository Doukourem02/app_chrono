...
export function payerPushCopy(raw: unknown): { title: string; body: string } | null {
  ...
  switch (status) {
    ...
    case 'picked_up':
      return { title: 'Colis récupéré', body: 'Le livreur a récupéré le colis.' };
    case 'delivering':
    ...
    case 'declined':
      return { title: 'Commande refusée', body: 'Votre commande n’a pas été acceptée.' };
    default:
      return null;
  }
}

export function recipientPushCopy(raw: unknown): { title: string; body: string } | null {
  const status = normalizeProductStatus(raw);
  switch (status) {
    case 'accepted':
    case 'enroute':
    ...
    case 'declined':
      return { title: 'Commande refusée', body: 'Cette livraison n’a pas été acceptée.' };
    default:
      return null;
  }
}

export function publicTrackCopy(raw: unknown): { title: string; body: string } | null {
  const status = normalizeProductStatus(raw);
  switch (status) {
    case 'pending':
