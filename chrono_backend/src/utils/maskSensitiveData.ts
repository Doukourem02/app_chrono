 ...
 * SÉCURITÉ: Ne jamais logger directement les emails, tokens, mots de passe, etc.
 */
...
/**
 * Masque un token JWT pour les logs
 * Exemple: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..." -> "eyJhbG***...xyz"
 */
 ...
 * Exemple: "+225 07 12 34 56 78" -> "+225 07 *** *** 78"
 */
...
/**
 * Alias pour maskPhone (compatibilité)
 */
 ...
 * Ne JAMAIS logger un mot de passe, même masqué
 */
...
/**
 * Masque un ID utilisateur pour les logs (optionnel)
 * Utile si les IDs sont sensibles
 */
...
/**
 * Masque une adresse IP pour les logs (optionnel)
 */
export function maskIP(ip: string | null | undefined): string {
  ...
  // Masquer les 2 derniers octets pour IPv4
  const parts = ip.split('.');
  if (parts.length === 4) {
    return `${parts[0]}.${parts[1]}.***.***`;
  }
  
  // IPv6: masquer les 4 derniers segments
...
/**
 * Masque un montant/prix pour les logs
 * Exemple: 357480 -> "*** FCFA"
 */
export function maskAmount(amount: number | string | null | undefined): string {
  ...
  // Ne pas logger les montants exacts
  return '*** FCFA';
}

/**
 * Masque un ID de commande pour les logs
 * Exemple: "5bf15c4f-c094-4555-81d8-50cb49012c6b" -> "5bf1***c6b"
 */
...
/**
 * Masque les statistiques financières pour les logs
 */
export function maskFinancialStats(stats: {
  ...
  if (masked.earningsByMethod) {
    masked.earningsByMethod = { moto: '***', vehicule: '***', cargo: '***' };
  }
...
/**
 * Nettoie un objet en masquant les champs sensibles
 */
export function sanitizeObject(obj: Record<string, any>): Record<string, any> {
  const sensitiveFields = ['password', 'token', 'accessToken', 'refreshToken', 'secret', 'apiKey', 'email', 'phone', 'userId', 'user_id', 'driver_id', 'price', 'price_cfa', 'totalEarnings', 'earnings'];
  const sanitized = { ...obj };
  ...
  for (const field of sensitiveFields) {
    if (sanitized[field] !== undefined) {
      ...
      } else if (field === 'userId' || field === 'user_id' || field === 'driver_id') {
        sanitized[field] = maskUserId(sanitized[field]);
      } else if (field === 'price' || field === 'price_cfa' || field.includes('Earnings') || field.includes('earnings')) {
