 ...
 * POST /api/qr-codes/manual
 */
export const manualVerifyQRCode = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ success: false, message: 'Non authentifié' });
    ...
    const { orderId, code, location, deviceInfo } = req.body;

    if (!orderId || !code) {
      res.status(400).json({ success: false, message: 'orderId et code sont requis.', code: 'QR_PAYLOAD_MISSING' });
      return;
    }
      ...
      if (io && orderId) {
        ...
        if (row?.user_id) {
          ...
          void notifyAllForOrderStatus({
            ...
            const msg = e instanceof Error ? e.message : String(e);
            logger.warn('[recipient-notify] manualVerifyQRCode:', msg);
          });
        }
      }
    } catch (socketErr: any) {
      logger.warn('[manualVerifyQRCode] Échec notification socket:', socketErr?.message);
    }

    res.json({ success: true, message: 'Code validé avec succès', data: result.data });
  } catch (error: any) {
    logger.error('Erreur lors de la vérification manuelle du code:', error);
    res.status(500).json({ success: false, message: 'Erreur serveur.', code: 'SCAN_SERVER_ERROR' });
  }
};
