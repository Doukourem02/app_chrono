...
async function recordStatusHistory(orderId: string, status: string, detail: any = {}): Promise<void> {
  try {
    ...
    logger.warn('⚠️ Impossible d\'enregistrer l\'historique du statut:', error.message);
  }
}
...
async function upsertOrderAssignment(
  ...
  try {
    ...
    const assigned = coerceDate(timestamps.assignedAt) || new Date();
    const accepted = coerceDate(timestamps.acceptedAt);
    const declined = coerceDate(timestamps.declinedAt);
    ...
    const updateResult = await (pool as any).query(updateQuery, [orderId, driverId, assigned, accepted, declined]);
...
export async function recordOrderAssignment(
  ...
  try {
    ...
    logger.warn('⚠️ Impossible de persister l\'affectation commande:', error?.message || error);
  }
}
  ...
  try {
    ...
    if (hasDriverColumn && !driverId && status === 'completed') {
      try {
        ...
        logger.warn(`⚠️ Erreur récupération driver_id pour order ${maskOrderId(orderId)}:`, err.message);
      }
    }
    ...
    if (hasDriverColumn && driverId) {
      ...
      logger.warn(`⚠️ hasDriverColumn=true mais driverId est null/undefined pour order ${maskOrderId(orderId)} en statut completed`);
    } else if (!hasDriverColumn) {
      logger.warn(`⚠️ Colonne driver_id n'existe pas dans orders pour order ${maskOrderId(orderId)}`);
    }
...
export async function getActiveOrdersByUser(userId: string): Promise<any[]> {
  try {
    ...
    logger.error('❌ Erreur lors de la récupération des commandes actives:', error);
    throw error;
  }
}
...
export async function getActiveOrdersByDriver(driverId: string): Promise<any[]> {
  try {
    ...
    logger.error('❌ Erreur lors de la récupération des commandes du chauffeur:', error);
    throw error;
  }
}
