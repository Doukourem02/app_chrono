...
export const ErrorTypes = {
  ...
  NETWORK: (onRetry?: () => void, operation?: string): ErrorModalData => {
    ...
    return {
      ...
      icon: 'wifi-outline',
      color: '#F59E0B',
      explanation: `Cette erreur survient lorsque votre appareil ne peut pas établir une connexion avec nos serveurs${operationMessage}. Cela peut être dû à une connexion internet instable, un problème réseau temporaire, ou un serveur temporairement indisponible.`,
      suggestions: [
  ...
  PERMISSION: (type: 'localisation' | 'photos' | 'caméra' = 'localisation'): ErrorModalData => {
    ...
    return {
      ...
      icon: type === 'localisation' ? 'location-outline' : type === 'photos' ? 'images-outline' : 'camera-outline',
      color: '#F59E0B',
      onClose: () => {},
    };
  },
  ...
  LOGIN_REQUIRED: (): ErrorModalData => ({
    ...
    onAction: () => {
      // Sera géré par UserFriendlyError
    },
    onClose: () => {},
  }),
  ...
  EXTERNAL_APP: (app: 'email' | 'téléphone' | 'whatsapp' | 'navigation'): ErrorModalData => {
    const configs = {
      email: {
        title: 'Application email non disponible',
        message: 'Impossible d\'ouvrir l\'application email.',
        explanation: 'Votre appareil ne peut pas ouvrir une application email. Cela peut être dû à l\'absence d\'une application email installée ou à une configuration incorrecte.',
        suggestions: [
          'Installez une application email (Gmail, Outlook, etc.)',
          'Vérifiez que votre application email est configurée',
          'Réessayez après installation',
        ],
      },
      téléphone: {
        title: 'Application téléphone non disponible',
        message: 'Impossible d\'ouvrir l\'application téléphone.',
        explanation: 'Votre appareil ne peut pas ouvrir une application téléphone. Cela peut être dû à l\'absence d\'une application téléphone installée ou à une configuration incorrecte.',
        suggestions: [
          'Vérifiez que votre téléphone est correctement configuré',
          'Réessayez',
        ],
      },
      whatsapp: {
    ...
    return {
      ...config,
      errorCode: `EXTERNAL_APP_${app.toUpperCase()}_ERROR`,
      icon: app === 'email' ? 'mail-outline' : app === 'téléphone' ? 'call-outline' : app === 'whatsapp' ? 'logo-whatsapp' : 'map-outline',
      color: '#F59E0B',
      onClose: () => {},
    };
  },
